...
iupdate(struct inode *ip)
{
  ...
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));  //el IBLOCK es un semaforo. bloquea esa parte del disco.
  ...
  dip->nlink = ip->nlink;
  dip->size = ip->size;
  memmove(dip->permissions, ip->permissions, 4); //mueve los permisos
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
  log_write(bp);
  brelse(bp); //hace liberación del disco - semaforo.
}
...
ilock(struct inode *ip)
{
  ...
  if(!(ip->flags & I_VALID)){
    ...
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs)); //mover de origen a destino
    brelse(bp);
    ip->flags |= I_VALID;
    if(ip->type == 0)
      panic("ilock: no type");
...
stati(struct inode *ip, struct stat *st)
{
  ...
  st->nlink = ip->nlink;
  st->size = ip->size;
  memmove(st->permissions, ip->permissions, 4); // va de ip para st
}
